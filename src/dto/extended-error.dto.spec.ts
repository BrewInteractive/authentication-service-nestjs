...
describe("MyExtendedError", () => {
  it("should correctly assign a message and extensions", () => {
    ...
    const extensions = MockFactory(ErrorExtensionsFixture)
      .mutate({ code: errorCode, additionalProperty: errorAdditionalProperty })
      .one();

    const extendedError = MockFactory(ExtendedErrorFixture)
      .mutate({ message: errorMessage, extensions })
      .one();
    ...
    expect(extendedError.extensions.additionalProperty).toBe(
      errorAdditionalProperty
    );
  });
